...
def genrate_ameca_prompt(explanation_level='beginner', enforce_length=True):
    ...
    return {
        ...
        "EXPERIMENT EXPECTATIONS FROM_USER": [
        ...
        "EXPLANATION LEVEL": f"{explanation_level}",
        "STRICT RULES": [
...
class RobotExpression:
    ...
    def set_emotion(
        ...
        # Use the same canonical Ekman mapping for participant-emotion metadata
        # and robot-response facial actuation.
        ...
        # Compare the ACTUAL physical sequence rather than only the semantic
        # emotion. For example sadness/anger/fear/disgust all intentionally map
        # to the same calm neutral sequence in the reference design.
        if not force and sequence_name == self.last_emotion:
            return None

        expected_duration = self._play_sequence(sequence_name)
        self.last_emotion = sequence_name
...
# Self-RAG activation phrases. ASR/user wording varies between
# "robotic research lab" and "robotics research lab", so both must route to
# the same lab-knowledge pipeline.
SELF_RAG_TRIGGER_PHRASE = "research lab"
SELF_RAG_TRIGGER_PATTERNS = [
    re.compile(r"\\brobotic(?:s)?\\s+research\\s+lab(?:oratory)?\\b", re.IGNORECASE),
    re.compile(r"\\brobot\\s+research\\s+lab(?:oratory)?\\b", re.IGNORECASE),
    re.compile(r"\\brrlab\\b", re.IGNORECASE),
]
...
def mentions_self_rag_trigger(text: str) -> bool:
    ...
    return any(pattern.search(normalized) for pattern in SELF_RAG_TRIGGER_PATTERNS)
