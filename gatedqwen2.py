...
VISUAL MODALITY FACE-PRESENCE GATE:
Candidate frames collected during the utterance are ordered by sharpness.
DeepFace checks them in that order until it confirms a face. Qwen2.5-VL
then analyzes the first confirmed-face frame. If DeepFace finds no face in
all candidates, the visual modality is unavailable for that turn.
"""
...
CAMERA_DEVICE: Optional[int] = int(os.environ.get("CAMERA_DEVICE", "1" if IS_LINUX else "0"))
...
_WHISPER_USE_CUDA = torch.cuda.is_available()
FAST_WHISPER_CONFIG = {
    "profile": "gpu" if _WHISPER_USE_CUDA else "cpu",
    "model": os.environ.get("WHISPER_MODEL", "base"),
    "device": os.environ.get("WHISPER_DEVICE", "cuda" if _WHISPER_USE_CUDA else "cpu"),
    "compute_type": os.environ.get(
        "WHISPER_COMPUTE_TYPE", "float16" if _WHISPER_USE_CUDA else "int8"
    ),
    "language": "en",
    "beam_size": 1,
    "vad_filter": False,
...
#
# CURRENT DESIGN: DeepFace is used only as a face-presence gate. Candidate
# frames are checked in descending sharpness order. The first frame with a
# confirmed face is passed to Qwen2.5-VL. DeepFace emotion scores are not
# blended into the visual emotion distribution.
...
class FaceEmotionCapture:
    ...
    # Diagnostics describing candidate-by-candidate DeepFace face-presence
    # checks and which frame, if any, was selected for Qwen2.5-VL.
    ...
    def as_json(self) -> dict:
        ...
        return {
            ...
            "note": (
                "Candidate frames are checked by DeepFace in descending sharpness "
                "order. Qwen2.5-VL analyzes the first candidate with a confirmed "
                "face. If no candidate contains a face, the visual modality is "
                "unavailable for the turn; see deepface_diagnostics."
            ),
        }
...
class RobotSpeaker:
    """Low-level synchronous Tritium TTS client used only by RobotOutputQueue."""

    def __init__(self, tts_url: str, tts_token: str = "", speaking_cooldown_s: float = 0.3) -> None:
        self.tts_url = tts_url
        self.tts_token = tts_token
        self.speaking_cooldown_s = max(0.0, speaking_cooldown_s)
        self._speaking_until = 0.0
        self._state_lock = threading.Lock()

    @staticmethod
    def estimate_speech_duration(text: str) -> float:
        # Ameca's voice is typically around 135-155 words/minute. The
        # additional cooldown protects against room echo after playback.
        word_count = max(1, len(text.split()))
        return max(1.0, word_count / 2.35 + 0.5)

    def bump_speaking_tail(self, extra: float) -> None:
        with self._state_lock:
            self._speaking_until = max(self._speaking_until, time.time() + max(0.0, extra))

    def reserve_echo_guard(self, text: str) -> None:
        self.bump_speaking_tail(self.estimate_speech_duration(text) + self.speaking_cooldown_s)
    ...
    def is_speaking_or_cooling_down(self) -> bool:
        ...
        if HAS_TTS_ACTIVITY_MONITOR:
            try:
                ...
                activity_flag = False
        with self._state_lock:
            guarded = time.time() < self._speaking_until
        return activity_flag or guarded
    ...
    def say_now(self, text: str) -> bool:
        ...
        self.reserve_echo_guard(spoken)
        ...
        try:
            response = requests.put(
                self.tts_url, data=spoken.encode("utf-8"), headers=headers, timeout=5
            )
            if 200 <= response.status_code < 300:
                print_ts(f"[TTS] Tritium responded {response.status_code} OK.")
                return True
            print_ts(
                f"[TTS] Tritium responded with status {response.status_code}: "
...
class RobotExpression:
    """Low-level synchronous Tritium expression client used by RobotOutputQueue."""
    ...
    def set_emotion_now(self, emotion: str, confidence: float = 1.0, force: bool = False) -> bool:
        ...
        if not force and not EXPRESSION_FORCE_REPLAY_SAME and resolved_emotion == self.last_emotion:
            print_ts(f"[EXPRESSION] Emotion unchanged ({resolved_emotion}); skipping replay.")
            return True
        ...
        try:
            ...
            print_ts(f"[EXPRESSION] PUT {uri} -> {response.status_code} {'OK' if ok else 'FAILED'}")
            if ok:
                self.last_emotion = resolved_emotion
            return ok
        except Exception as exc:
            print_ts(f"[EXPRESSION] Failed to play '{sequence_name}': {exc}")
            return False


@dataclass
class RobotOutputCommand:
    kind: str
    text: str = ""
    emotion: str = "trust"
...
class RobotOutputQueue:
    """
    Dedicated non-blocking robot-output queue. Network delays in Tritium TTS
    or sequence playback never block ASR, fusion, retrieval, or LLM work.
    """

    def __init__(self, speaker: RobotSpeaker, expression: RobotExpression, expression_enabled: bool = True) -> None:
        self.speaker = speaker
        self.expression = expression
        self.expression_enabled = expression_enabled
        self._queue: "queue.Queue[Optional[RobotOutputCommand]]" = queue.Queue(maxsize=64)
        self._stop_event = threading.Event()
        self._thread = threading.Thread(target=self._run, name="robot-output", daemon=True)
        self._thread.start()
    ...
    def say(self, text: str) -> None:
        ...
        if not spoken:
            return
        # Reserve the guard before the worker reaches Tritium, so the listener
        # cannot start during queueing/network delay and hear the robot itself.
        self.speaker.reserve_echo_guard(spoken)
        self._put(RobotOutputCommand(kind="speech", text=spoken))

    def set_expression(self, emotion: str, confidence: float = 1.0, force: bool = False) -> None:
        if self.expression_enabled:
            self._put(RobotOutputCommand(
                kind="expression", emotion=emotion, confidence=confidence, force=force
            ))

    def is_speaking_or_cooling_down(self) -> bool:
        return self.speaker.is_speaking_or_cooling_down() or not self._queue.empty()

    def _put(self, command: RobotOutputCommand) -> None:
        try:
            self._queue.put_nowait(command)
        except queue.Full:
            print_ts(f"[ROBOT OUTPUT] Queue full; dropping {command.kind} command.")
    ...
    def _run(self) -> None:
        while not self._stop_event.is_set():
            ...
            try:
                if command is None:
                    return
                if command.kind == "speech":
                    self.speaker.say_now(command.text)
                elif command.kind == "expression" and self.expression_enabled:
                    self.expression.set_emotion_now(
                        command.emotion, command.confidence, command.force
                    )
            except Exception as exc:
                print_ts(f"[ROBOT OUTPUT] Command failed: {exc}")
            finally:
                self._queue.task_done()

    def shutdown(self, drain: bool = True, timeout: float = 8.0) -> None:
        if drain:
...
def choose_robot_expression(user_emotion: EmotionResult) -> RobotExpressionDecision:
    """Select a socially appropriate robot expression without altering detected user emotion."""
    policy = {
        "joy": ("joy", "Mirror positive affect."),
        "trust": ("trust", "Maintain a warm attentive expression."),
        "surprise": ("surprise", "Acknowledge surprise without amplification."),
        "sadness": ("sadness", "Show gentle empathic concern."),
        "anticipation": ("anticipation", "Show attentive interest."),
        "fear": ("trust", "Use reassurance rather than mirroring fear."),
        "anger": ("trust", "Use a calm expression rather than confrontation."),
        "disgust": ("trust", "Avoid appearing judgemental or repulsed."),
        "neutral": ("trust", "Use the neutral attentive expression."),
    }
    robot_emotion, reason = policy.get(user_emotion.emotion, policy["neutral"])
    return RobotExpressionDecision(
        user_emotion=user_emotion.emotion,
...
class DeepFaceClient:
    ...
    def _start_worker(self) -> bool:
        ...
        if not got_ready:
            print_ts(
                ...
                f"(check DEEPFACE_PYTHON='{self.python_executable}' has deepface+tensorflow installed)."
            )
            self._ready = False
    ...
    def analyze(self, frame_bgr: np.ndarray) -> Optional[DeepFaceResult]:
        """Analyze one frame through the isolated worker, one request at a time."""
        if not self.is_alive():
            return None
        if frame_bgr is None or not isinstance(frame_bgr, np.ndarray) or frame_bgr.size == 0:
            print_ts("[DeepFace] Refusing to analyze an empty frame.")
            return None

        # The complete request/response transaction is serialized. This
        # prevents concurrent callers from consuming one another's replies.
        with self._lock:
            temp_path = None
            try:
                ...
                if not cv2.imwrite(temp_path, frame_bgr):
                    print_ts(f"[DeepFace] Failed to write temporary image: {temp_path}")
                    return None
                ...
                try:
                    ...
                    print_ts(f"[DeepFace] Could not send request: {exc}")
                    self._ready = False
                    return None
                ...
                while time.time() < deadline:
                    ...
                    if response.get("request_id") != request_id:
                        print_ts("[DeepFace] Ignoring stale worker response.")
                        continue
                    if not response.get("ok"):
                        print_ts(f"[DeepFace] Worker error: {response.get('error')}")
                        return None
    ...
    transcript_data = {
        ...
        "session": {
            ...
            "face_emotion": {
                ...
                "face_analysis_strategy": "first_confirmed_face_candidate_ordered_by_sharpness",
                ...
                "hybrid_deepface": {
                    ...
                    "face_presence_gate_order": "deepface_candidates_by_sharpness_then_vlm_conditional",
                    "note": (
                        "DeepFace checks candidate frames in descending sharpness order. "
                        "Qwen2.5-VL analyzes the first candidate with a confirmed face. "
                        "If DeepFace is unavailable, the sharpest candidate is analyzed "
                        "ungated. If no candidate contains a face, visual analysis is skipped. See deepface_diagnostics on each turn's "
                        "face_emotion for whether the gate passed."
                    ),
                },
            },
            "vad": {
...
class FaceEmotionSampler:
    """
    Collect candidate frames from the shared ZED camera while the user speaks.
    Candidates are ranked by Laplacian sharpness. DeepFace checks them in that
    order and Qwen2.5-VL analyzes the first frame with a confirmed face. If the
    DeepFace worker is unavailable, the sharpest frame is analyzed ungated.
    """
    ...
    def __init__(
        ...
        # Total wall-clock seconds spent checking candidate frames with DeepFace.
        self.deepface_analysis_seconds: Optional[float] = None
        # Candidate-selection diagnostics surfaced in FaceEmotionCapture.
    ...
    def _analyze_snapshot_now(self) -> None:
        """
        Check candidate frames in descending sharpness order with DeepFace.
        Qwen2.5-VL receives the first candidate with a confirmed face. If
        DeepFace is unavailable, the sharpest frame is analyzed ungated.
        """
        if not self.candidate_frames:
            self.last_status = "no frames captured"
            return

        ordered_candidates = sorted(self.candidate_frames, key=lambda item: item[0], reverse=True)
        selected: Optional[tuple[int, float, np.ndarray]] = None
        checks: list[dict[str, Any]] = []
        deepface_available = self.deepface_client is not None and self.deepface_client.is_alive()
        deepface_total_seconds = 0.0

        if deepface_available:
            for rank, (sharpness, frame) in enumerate(ordered_candidates, start=1):
                started = time.perf_counter()
                result = self.deepface_client.analyze(frame)
                elapsed = time.perf_counter() - started
                deepface_total_seconds += elapsed
                checks.append({
                    "rank": rank,
                    "sharpness": round(sharpness, 3),
                    "available": result is not None,
                    "no_face": bool(result and result.no_face),
                    "seconds": round(elapsed, 4),
                })
                if result is None:
                    # Worker unavailable/timed out: use the current sharpest
                    # remaining candidate ungated rather than losing vision.
                    selected = (rank, sharpness, frame)
                    break
                if not result.no_face:
                    selected = (rank, sharpness, frame)
                    break
        else:
            sharpness, frame = ordered_candidates[0]
            selected = (1, sharpness, frame)

        self.deepface_analysis_seconds = deepface_total_seconds if deepface_available else None
        self.deepface_diagnostics = {
            "deepface_available": deepface_available,
            "selection_strategy": "first_face_candidate_ordered_by_sharpness",
            "candidate_count": len(ordered_candidates),
            "candidate_checks": checks,
        }

        if selected is None:
            self.last_status = "no face detected in candidates"
            self.analysis_seconds = 0.0
            self.deepface_diagnostics["vlm_skipped_reason"] = (
                "DeepFace found no face in any sampled candidate frame."
            )
            print_ts("[DeepFace] No face found in any candidate; skipping Qwen2.5-VL.")
            return

        selected_rank, selected_sharpness, self.snapshot_frame = selected
        self.deepface_diagnostics["selected_rank"] = selected_rank
        self.deepface_diagnostics["selected_sharpness"] = round(selected_sharpness, 3)
        self.deepface_diagnostics["vlm_gated"] = deepface_available and bool(checks and checks[-1]["available"])

        print_ts(
            f"Analyzing candidate rank {selected_rank} with Qwen2.5-VL "
            f"(sharpness={selected_sharpness:.1f}, candidates={len(ordered_candidates)})..."
        )
        ...
        ordered = sorted(vlm_scores.items(), key=lambda item: item[1], reverse=True)
        top_emotion, top_score = ordered[0]
        second_score = ordered[1][1] if len(ordered) > 1 else 0.0
        if top_score < FACE_MIN_TOP_SCORE or (top_score - second_score) < FACE_MIN_MARGIN:
            self.last_status = f"weak/mixed result: {top_emotion}={top_score:.0f}%"
            print_ts(f"Facial result rejected: {self.last_status}")
            return

        self.emotion_score_samples.append(vlm_scores)
        self.last_status = f"{top_emotion}: {top_score:.0f}%"
        print_ts(f"Facial result accepted: {self.last_status}")
...
def face_reliability_score(face_emotion_json: Optional[dict]) -> float:
    ...
    frame_component = min(1.0, frame_count / max(1.0, float(FACE_MAX_CANDIDATE_FRAMES)))
    ...
    # Under the current candidate face-presence-gate design,
...
def parse_robot_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--disable_deepface",
        action="store_true",
        help="Disable the DeepFace face-presence gate even if --deepface_python is set (Qwen2.5-VL will then run on every utterance's sharpest frame with no gating).",
    )
    parser.add_argument(
        "--deepface_timeout",
        type=float,
        ...
        help=(
            f"DeepFace analyze() timeout in seconds for the candidate face-presence checks "
            ...
            "backend like mtcnn) -- a timeout means DeepFace is treated as unavailable "
            "for that turn, which also disables the face-presence gate ahead of "
            "Qwen2.5-VL for that turn, since there is no confirmed face/no-face result "
            "to gate on (Qwen2.5-VL then runs ungated on the sharpest frame)."
        ),
    )
...
def main() -> None:
    ...
    # favor of the isolated candidate face-presence-gate DeepFace usage
    # (DeepFaceClient / deepface_worker.py, launched in its own
    # process/conda env further down in this function), which is what
    # actually gates Qwen2.5-VL in this pipeline. Self-RAG's embeddings
    # also go through Ollama instead of an in-process
    ...
    # ---- Dedicated non-blocking robot-output queue ----
    robot_output = RobotOutputQueue(
        ...
        expression_enabled=not args.disable_expression,
    )
    # Existing listener/name-capture helpers accept a speaker-like object;
    # RobotOutputQueue intentionally exposes say() and echo-guard methods.
    robot_speaker = robot_output

    # ---- Visual modality: DeepFace candidate face-presence gate + Qwen2.5-VL ----
    ...
    # call in FaceEmotionSampler._analyze_snapshot_now() degrades to
    # calling Qwen2.5-VL directly on the sharpest frame with no gate.
    # DeepFace checks candidates in descending sharpness order; Qwen analyzes
    # the first frame with a confirmed face.
    deepface_client: Optional[DeepFaceClient] = None
    ...
    if deepface_enabled:
        ...
        if deepface_client.is_alive():
            print_ts("DeepFace candidate face-presence gate ENABLED (checks candidates ordered by sharpness).")
        else:
            print_ts("DeepFace face-presence gate unavailable this session; Qwen2.5-VL will run ungated on the sharpest candidate.")
    else:
        print_ts("DeepFace face-presence gate disabled (DEEPFACE_ENABLED=0 or --disable_deepface); Qwen2.5-VL will run ungated.")
    ...
    if not args.disable_expression:
        robot_output.set_expression("trust", confidence=1.0, force=True)
    ...
    print(
        "Speak naturally. Silero VAD will detect speech; DeepFace will check sampled frames "
        "in descending sharpness order, and Qwen2.5-VL will analyze the first frame with a "
        "confirmed face; faster-whisper will transcribe the utterance; adaptive reliability-aware "
        "fusion will combine text, visual, and prosody signals; a separate expression policy "
        "will select Ameca's physical facial expression; and TTS will be dispatched through "
        "the dedicated robot-output queue."
    )
    ...
    try:
        while True:
            ...
            if maybe_name and not looks_like_invalid_name(maybe_name):
                ...
                if not args.disable_expression:
                    robot_output.set_expression("trust", confidence=1.0)
                print()
            ...
            try:
                ...
                # Keep detected user emotion separate from the robot's socially
                # appropriate expression policy. The user emotion still controls
                # response conditioning; only the physical face uses this decision.
                robot_expression_decision = choose_robot_expression(emotion_result)
                if not args.disable_expression:
                    robot_output.set_expression(
                        robot_expression_decision.robot_emotion,
                        confidence=robot_expression_decision.confidence,
                    )
                ...
                emotion_json["robot_expression_decision"] = {
                    "detected_user_emotion": robot_expression_decision.user_emotion,
                    "robot_expression": robot_expression_decision.robot_emotion,
                    "confidence": robot_expression_decision.confidence,
                    "reason": robot_expression_decision.reason,
                }
                ...
                print_ts(
                    f"Smoothed detected user emotion used for response tone: {emotion_result.emotion} "
                ...
                user_message = {
                    ...
                    "input_mode": "silero_vad_faster-whisper_qwen25vl_candidate_deepface_gate_adaptive_reliability_aware_fusion_temporal_smoothing_self_rag",
                }
        ...
        try:
            robot_output.shutdown(drain=True)
            print_ts("[ROBOT OUTPUT] Queue shut down.")
        except Exception:
            pass
