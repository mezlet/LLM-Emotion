...
class SessionVideoRecorder:
    ...
    def _run(self) -> None:
        ...
        while not self._stop.is_set():
            ...
            # Catch up to real elapsed time by writing (possibly duplicate)
            # frames on a fixed schedule, rather than gating on new-frame
            # arrival. This keeps the file's declared fps in sync with wall
            # clock time even if the camera delivers frames slower than fps
            # (e.g. while sharing the device with a FrameCollector), which is
            # what was causing sped-up playback: frames were being stamped
            # 1/fps apart in the file even when they actually arrived farther
            # apart in real time.
            now = time.monotonic()
