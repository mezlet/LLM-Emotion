...
class FaceEmotionCapture:
    ...
    def as_json(self) -> dict:
        ...
        return {
            ...
            "note": (
                "DeepFace checks only the single sharpest sampled frame for this "
                "utterance, as a face-presence gate. If DeepFace confirms a face, "
                "Qwen2.5-VL analyzes that exact same single frame directly (no "
                "separate DeepFace crop is passed to Qwen, and no other candidate "
                "frames are considered as a fallback). If DeepFace reports no face "
                "on the sharpest frame, Qwen2.5-VL is not called at all and the "
                "visual modality contributes no signal for this turn; see "
                "deepface_diagnostics for details."
            ),
        }
...
def deepface_scores_to_plutchik(deepface_result: Optional[DeepFaceResult]) -> Optional[dict[str, float]]:
    ...
    NOTE: no longer called from FaceEmotionSampler under the single-frame
    face-presence-gate design (DeepFace's role there is purely presence
    detection, not emotion scoring). Kept for potential reuse (e.g. if a
    future revision wants to log or blend DeepFace's own emotion reading
...
def combine_vlm_and_deepface_scores(
    ...
    single-frame face-presence-gate design. Previously this function did
    double duty -- both gating (discarding VLM's reading when DeepFace
    found no face) and blending (weighting VLM vs DeepFace scores together
    when both found a face). Under the simplified design, DeepFace checks
    only the single sharpest frame BEFORE Qwen2.5-VL is ever called: if no
    face is found, Qwen is skipped entirely (no VLM scores exist to
    discard); if a face is found, Qwen analyzes that same frame and its
    scores are used directly, with no DeepFace-score blending. This
    function is kept for potential reuse/logging (e.g. if a future
    revision wants DeepFace's own emotion reading folded back in) but is
    dead code on the current call path.
    ...
    transcript_data = {
        ...
        "session": {
            ...
            "face_emotion": {
                ...
                "face_analysis_strategy": "single_sharpest_frame_deepface_gated",
                ...
                "hybrid_deepface": {
                    ...
                    "face_presence_gate_order": "deepface_single_sharpest_frame_then_vlm_conditional",
                    "note": (
                        "DeepFace checks ONLY the single sharpest candidate frame from the "
                        "utterance. Qwen2.5-VL is only called (on that same single frame) if "
                        "DeepFace confirms a face is present, or if DeepFace is unavailable "
                        "this turn. If DeepFace finds no face on the sharpest frame, "
                        "Qwen2.5-VL is skipped entirely for that turn, with no fallback to "
                        "any other candidate frame. See deepface_diagnostics on each turn's "
...
class FaceEmotionSampler:
    """
    Facial-expression sampler feeding Qwen2.5-VL, gated by a single-frame
    DeepFace face-presence check.
    ...
    sharpness. Once speech ends, ONLY the single sharpest of those
    collected frames is used:

    1. DeepFace checks that one frame for face presence (a fast,
       dedicated face detector, run in an isolated process -- see
       DeepFaceClient).
    2. If DeepFace confirms a face (or is unavailable this turn), that
       exact same frame is sent to Qwen2.5-VL for emotion classification.
    3. If DeepFace reports no face on that frame, Qwen2.5-VL is not
       called at all for this turn -- there is no fallback to the
       second-sharpest (or any other) candidate frame. "No face on the
       sharpest frame" always means an immediate step-down of the visual
       modality for that turn.

    This keeps the face-presence guarantee unambiguous: it always
    describes exactly the one frame that was (or wasn't) sent to Qwen,
    never a batch where only one frame was actually checked.
    """
    ...
    def __init__(
        ...
        # Wall-clock seconds spent inside the DeepFace analyze() call on
        # the single sharpest frame. None if DeepFace was not used this
        # turn.
        self.deepface_analysis_seconds: Optional[float] = None
        # Diagnostics from the single-frame DeepFace face-presence gate;
        # surfaced on the resulting FaceEmotionCapture for transparency/
        # debugging.
    ...
    def _analyze_snapshot_now(self) -> None:
        """
        Analyze the single sharpest candidate frame captured during this
        utterance, gated by a single-frame DeepFace face-presence check.

        Design: DeepFace checks ONLY the single sharpest frame -- not all
        collected candidates, and not a multi-frame batch. If DeepFace
        confirms a face on that frame (or DeepFace is unavailable this
        turn), Qwen2.5-VL analyzes that exact same frame directly -- no
        separate DeepFace crop is passed to Qwen, and no other candidate
        frame is considered. If DeepFace reports no face on the sharpest
        frame, Qwen2.5-VL is not called at all for this turn and the
        visual modality contributes no signal to fusion; there is no
        fallback to a second-sharpest frame. "No face on the sharpest
        frame" always means an immediate step-down for the turn, full
        stop.
        """
        ...
        # ---- DeepFace checks ONLY the single sharpest frame ----
        ...
        self.deepface_diagnostics = {
            "deepface_available": deepface_result is not None,
            "deepface_no_face": bool(deepface_result and deepface_result.no_face),
            "frame_checked": "single_sharpest",
            "candidates_considered": len(self.candidate_frames),
        }

        # ---- Step down the visual modality if DeepFace found no face ----
        # No fallback to the second-sharpest (or any other) candidate
        # frame: "no face on the sharpest frame" always means an
        # immediate step-down for this turn, full stop.
        if deepface_result is not None and deepface_result.no_face:
            print_ts(
                "[DeepFace] No face detected in the sharpest candidate frame; "
                "skipping Qwen2.5-VL entirely for this turn (visual modality stepped down)."
            )
            self.last_status = "no face detected (DeepFace gate)"
            self.analysis_seconds = 0.0
            self.deepface_diagnostics["vlm_discarded_reason"] = (
                "DeepFace found no face in the single sharpest candidate frame; "
                "Qwen2.5-VL was not called this turn, and no other candidate frame "
                "was tried as a fallback."
            )
            return

        # ---- DeepFace confirmed a face (or was unavailable) -> send exactly ONE frame to Qwen ----
        print_ts(
            f"Analyzing single DeepFace-verified frame with Qwen2.5-VL "
            f"(sharpness={best_sharpness:.1f}, candidates_considered={len(self.candidate_frames)})..."
        )
        ...
        top_emotion, top_score = max(vlm_scores.items(), key=lambda item: item[1])
        if top_score < FACE_MIN_TOP_SCORE:
            print_ts(
                f"Facial result rejected as weak: "
                f"{top_emotion}={top_score:.1f}% < {FACE_MIN_TOP_SCORE:.1f}%"
            )
            self.last_status = f"weak result: {top_emotion}={top_score:.0f}%"
            return

        self.emotion_score_samples.append(vlm_scores)
        self.last_status = f"{top_emotion}: {top_score:.0f}%"
        print_ts(f"Facial result accepted (DeepFace-verified single frame): {self.last_status}")
...
def parse_robot_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            f"DeepFace analyze() timeout in seconds for the single sharpest-frame check "
...
def main() -> None:
    ...
    # favor of the isolated, single-frame face-presence-gate DeepFace usage
    ...
    # ---- Visual modality: DeepFace single-frame face-presence gate + Qwen2.5-VL ----
    ...
    # DeepFace checks ONLY the single sharpest candidate frame per
    # utterance -- see FaceEmotionSampler._analyze_snapshot_now().
    deepface_client: Optional[DeepFaceClient] = None
    ...
    if deepface_enabled:
        ...
        if deepface_client.is_alive():
            print_ts("DeepFace single-frame face-presence gate ENABLED (checks only the sharpest sampled frame per utterance).")
        else:
            print_ts("DeepFace face-presence gate unavailable this session; Qwen2.5-VL will run ungated on the sharpest frame.")
    else:
    ...
    print(
        "Speak naturally. Silero VAD will detect speech; DeepFace will check the single sharpest "
        "captured frame for a face first, and Qwen2.5-VL will only analyze that exact frame if a "
        "face is confirmed present; faster-whisper will transcribe the utterance; adaptive "
        "reliability-aware fusion (with cross-turn temporal smoothing) will combine "
        "text/visual/prosody; Ameca's face will update to match the smoothed emotion via Tritium "
        "sequence_player; and Ameca will respond out loud via Tritium TTS."
    )
