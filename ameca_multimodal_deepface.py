...
ZED_STEREO_WIDTH = 4416
ZED_STEREO_HEIGHT = 1242
...
def deepface_scores_to_plutchik(deepface_result: Optional[DeepFaceResult]) -> Optional[dict[str, float]]:
    ...
    plutchik_scores: dict[str, float] = {emo: 0.0 for emo in PLUTCHIK_EMOTIONS}  # was: if emo != "neutral"
...
def prompt_for_user_name(
    ...
    if participant_id.strip():
        user_key = slugify_name(participant_id)
        print_ts(f"Using fixed participant ID as storage key: {user_key} (spoken name: {final_name})")
    else:
    ...
    if is_new_user:
        ...
        users[user_key]["name"] = final_name
        users[user_key]["last_seen"] = now_ts()
...
def _looks_like_unparsed_json_schema(text: str) -> bool:
    """
    True if `text` is raw text that was clearly intended as the
    {"reply": ..., "emoji": ..., "tone": ...} JSON schema but failed to
    parse (e.g. truncated mid-string by num_predict). Speaking this
    verbatim to the user would leak literal JSON syntax over TTS, which
    is worse than retrying or falling back to a generic apology.
    """
    stripped = str(text or "").strip()
    if not stripped.startswith("{"):
        return False
...
def parse_robot_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Fixed participant identifier for the user study. When set, this becomes the "
        "storage key for users.json/session transcripts (stable across sessions and "
        "immune to ASR name-transcription drift), while Ameca still addresses the "
        "person by whatever name they actually say.",
)
...
def main() -> None:
    ...
    try:
        while True:
            ...
            if maybe_name and not looks_like_invalid_name(maybe_name) and not args.participant_id.strip():
