...
# TensorFlow / DeepFace quality-of-life settings.
...
# Good balance for DeepFace and preview. Use 3840x1080 only if your machine handles it smoothly.
...
class FaceEmotionCapture:
    ...
    def mapped_plutchik_emotion(self) -> Optional[str]:
        ...
        if not dominant:
            return None
        return DEEPFACE_TO_PLUTCHIK.get(dominant)
    ...
    def summary_text(self) -> str:
        ...
        if not self.is_reliable:
            ...
            return f"Facial-expression hint was weak or mixed; top_signals=({', '.join(top_parts)}); samples={self.sampled_frame_count}"
    ...
    def as_json(self) -> dict:
        ...
        return {
            ...
            "error": self.error,
            "note": "Facial expression analysis is a secondary signal. Fusion combines it with text emotion.",
        }
...
def check_ollama_available(max_attempts: int = 10, delay_seconds: float = 1.0) -> None:
    ...
    raise RuntimeError(f"Could not connect to Ollama at {OLLAMA_HOST} after {max_attempts} attempts.")
    ...
    def sample_if_due(self) -> None:
        ...
        if CAMERA_PREVIEW_ENABLED and self.window_created:
            try:
                ...
                cv2.putText(preview_frame, f"Emotion: {self.last_status}", (20, 70), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2, cv2.LINE_AA)
...
def listen_for_utterance_with_silero_vad_and_face_emotion(input_device: Optional[int], silero_model, camera_device: Optional[int] = CAMERA_DEVICE, prompt_label: str = "utterance") -> Tuple[Optional[str], Optional[FaceEmotionCapture]]:
    input_sample_rate = get_input_samplerate(input_device)
    input_block_size = max(1, int(input_sample_rate * 0.05))
    audio_queue: queue.Queue[np.ndarray] = queue.Queue()
    vad_iterator = VADIterator(silero_model, threshold=SILERO_THRESHOLD, sampling_rate=SILERO_SAMPLE_RATE, min_silence_duration_ms=SILERO_MIN_SILENCE_DURATION_MS, speech_pad_ms=SILERO_SPEECH_PAD_MS)
...
def save_session_transcript(user_key: str, user_profile: dict, session_log: list[dict]) -> str:
    ...
    transcript_data = {
        ...
        "session": {
            ...
            "face_emotion": {"backend": "DeepFace", "detector_backend": DEEPFACE_DETECTOR_BACKEND, "actions": DEEPFACE_ACTIONS},
...
def main() -> None:
    ...
    print("Speak naturally. Silero VAD will detect speech, DeepFace will sample facial emotion, text+face will be fused, and Ameca will respond.")
    ...
    try:
        while True:
            wav_path, face_capture = listen_for_utterance_with_silero_vad_and_face_emotion(INPUT_DEVICE, silero_model, CAMERA_DEVICE, "utterance")
            ...
            user_message = {
                ...
                "input_mode": "silero_vad_faster_whisper_deepface_multimodal_fusion",
            }
